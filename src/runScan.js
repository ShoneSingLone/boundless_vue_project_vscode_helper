...
const {
	analysisCommonVaribles,
	analysisVueVaribles
} = require("./analysisVaribles");
...
class Scanner {
	...
	async loadOneFile(file, isLastOne) {
		...
		if (isLastOne) {
			vscode.window.showInformationMessage(
				`"boundless-vue-helper" Complete${this.spend}`
			);
		}
	}
...
async function analysisVueFile({
	fileInfo,
	fileName,
	ext,
	urlInSourceCode,
	...
	if (identity) {
		const varibles = await analysisVueVaribles({
			fsPath: fileInfo.fsPath,
			identity,
			documentUriPath: fileInfo.path
		});
