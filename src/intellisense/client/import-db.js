...
class ImportDb {
     ...
     * @memberOf ImportDb
     */
    static saveImport(fileName, fileContentString, fileInfo, isDefault = false, discovered) {
        fileName = fileName.trim();
        if (fileName === '' || fileName.length === 1) {
            return;
        }
        let obj = new ImportObject(fileName, fileInfo, isDefault, discovered);
        let exists = ImportDb.imports.findIndex(m => m.name === obj.fileName && m.file.fsPath === fileInfo.fsPath);
        if (exists === -1) {
